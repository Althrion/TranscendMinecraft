...
public class TutorialTeleporter extends Teleporter {
				...
				for(int k1 = -2; k1 <= 2; ++k1) {
					for(int l1 = -1; l1 <= 3; ++l1) {
						int i2 = i + k1,
							...
							k2 = k - j1;
						boolean flag = l1 < 0;
						this.world.setBlockState(new BlockPos(i2, j2, k2), flag ? BlockInit.DED_WOOD.getDefaultState() : Blocks.AIR.getDefaultState());
					}
				}
			}
	...
	public boolean placeInExistingPortal(Entity entity, float rotationYaw) {
			...
			for(int i1 = -128; i1 <= 128; ++i1) {
				...
				for(int j1 = -128; j1 <= 128; ++j1) {
					...
					for(BlockPos pos1 = pos3.add(i1, this.world.getActualHeight() - 1 - pos3.getY(), j1); pos1.getY() >= 0; pos1 = pos2) {
						...
						if(this.world.getBlockState(pos1).getBlock() == BlockInit.PORTAL_1) {
							for(pos2 = pos1.down(); this.world.getBlockState(pos2).getBlock() == BlockInit.PORTAL_1; pos2 = pos2.down()) pos1 = pos2;
		...
		if(d0 >= 0.0D) {
			...
			PatternHelper helper = BlockInit.PORTAL_1.createPatternHelper(world, pos);
			...
			for(int j7 = -1; j7 <= 1; ++j7) {
				for(int l7 = 1; l7 < 3; ++l7) {
					for(int k8 = -1; k8 < 3; ++k8) {
						...
						boolean flag = k8 < 0;
						this.world.setBlockState(new BlockPos(k9, k10, k11), flag ? BlockInit.DED_WOOD.getDefaultState() : Blocks.AIR.getDefaultState());
					}
				}
			}
		}

		IBlockState state = BlockInit.PORTAL_1.getDefaultState().withProperty(DimOnePortal.AXIS, l6 == 0 ? Axis.Z : Axis.X);
		...
		for (int i8 = 0; i8 < 4; ++i8) {
			for (int l8 = 0; l8 < 4; ++l8) {
				for (int l9 = -1; l9 < 4; ++l9) {
					...
					this.world.setBlockState(new BlockPos(l10, l11, k12), flag1 ? BlockInit.DED_WOOD.getDefaultState() : state, 2);
				}
			}
