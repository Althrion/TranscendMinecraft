package com.althrion.mod.classes.blocks.unobtainable;

import java.util.Random;

import javax.annotation.Nullable;

import com.althrion.mod.classes.world.dimension.TeleporterOne;
...
public class DimOnePortal extends BlockBreakable {
	...
	protected static final AxisAlignedBB X_AABB = new AxisAlignedBB(0.0D, 8.0D, 0.375D, 1.0D, 1.8D, 0.625D);
	protected static final AxisAlignedBB Z_AABB = new AxisAlignedBB(0.375D, 0.0D, 0.8D, 0.625D, 1.0D, 1.8D);
	...
	public DimOnePortal() {
		...
		this.setRegistryName("portal_1");
		this.setUnlocalizedName("portal_1");
	}
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
		if(!entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn.isNonBoss() && entityIn instanceof EntityPlayerMP) {
			...
			} else if (player.dimension != DimensionInit.SHADOWLANDS.getId()) {
				player.timeUntilPortal = 10;
				//player.mcServer.getPlayerList().transferPlayerToDimension(player, DimensionInit.SHADOWLANDS.getId(), new TeleporterOne(player.mcServer.getWorld(DimensionInit.SHADOWLANDS.getId())));
			} else {
				player.timeUntilPortal = 10;
				//player.mcServer.getPlayerList().transferPlayerToDimension(player, 0, new TeleporterOne(player.mcServer.getWorld(0)));
			}
		}
	}
	...
	public static class Size {
		...
		protected int getDistanceUntilEdge(BlockPos pos, EnumFacing facing) {
			...
			for(i = 0; i < 22; ++i) {
				...
				if(!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock()) || this.world.getBlockState(blockpos.down()).getBlock() != BlockInit.DED_WOOD) break;
			}
			
			Block block = this.world.getBlockState(pos.offset(facing, i)).getBlock();
			return block == BlockInit.DED_WOOD ? i : 0;
		}
		...
		protected int calculatePortalHeight() {
			...
			for(this.height = 0; this.height < 21; ++this.height) {
				for(int i = 0; i < this.width; ++i) {
					...
					if(block == BlockInit.PORTAL_1) ++this.portalBlockCount;
					
					if(i == 0) {
						block = this.world.getBlockState(pos.offset(leftDir)).getBlock();
						if(block != BlockInit.DED_WOOD) break label56;
			...
			for(int j = 0; j < this.width; ++j) {
				if(this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != BlockInit.DED_WOOD) {
					this.height = 0;
					break;
				}
			}
		...
		protected boolean isEmptyBlock(Block block) {
			return block.getDefaultState().getMaterial() == Material.AIR || block == Blocks.FIRE || block == BlockInit.PORTAL_1;
		}
		...
		public void placePortalBlocks() {
			for (int i = 0; i < this.width; i++) {
				...
				for (int j = 0; j < this.height; j++) this.world.setBlockState(pos.up(j), BlockInit.PORTAL_1.getDefaultState().withProperty(DimOnePortal.AXIS, this.axis), 2);
			}
		}
